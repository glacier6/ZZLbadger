...
(1)目前badger的大致优化方向：
  ...
  3.待看BadgerDB缓存是缓存整个KV对还是K OFFSET对，如果是前者的话可以考虑优化成K OFFSET对，即使其变成一个类似跳表的东西（用NVM实现）。然后这个东西还可以在GC的时候用（因为只存K OFFSET的话能存特别多KV对）
...
(2)BadgerDB的一些需要关注点
  ...
  5.单个事务层面，需要记录自己读写的 key 列表，以及事务的开始时间戳（readTs，用于读取）和提交时间戳（commitTs，用于加在写入的key后），这部分信息维护在 Txn 结构体中
    全局事务层面，需要管理全局时间戳，以及最近提交的事务列表，用于在新的事务提交中对事务开始与提交时间戳中间提交过的事务范围进行冲突检查，乃至当前活跃的事务的最小时间戳，用于清理旧事务信息，这部分信息维护在 oracle 结构体中
  6.只读事务是不进行提交操作的，所以没有commitTs
  7.TODO: 事务获取commitTs的时候是否还没有读取数据或者写入数据（即真正的数据操作），如果没有，就很多事情好理解了
      反正现在至少写入数据是在获取commitTs之后，读取应该是在获取commitTs之前
      
    TODO:  那个事务过程笔记中，如果事务4是写入A事务，3是进行读取A的事务，会怎样（即写后读会咋样）
(3)一些简称
